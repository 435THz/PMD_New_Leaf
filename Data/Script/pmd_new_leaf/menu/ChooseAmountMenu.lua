...
function ChooseAmountMenu:initialize(x, y, width, height, title, start_number, min, max, callback)
    ...
    self.max = max
    self.start_number = math.clamp(min, start_number, max)
    ...
    if max~=0 then self.digits = math.max(math.floor(math.log(math.abs(max),10))) end
...
function ChooseAmountMenu:change_number(vector)
    ...
    num = math.clamp(self.min,num + change, self.max)
